...
public class SqlManager {
    ...
    public static void addPlot(Plot plot, int idX, int idZ, int topX, int bottomX, int topZ, int bottomZ) {
        Location top = PlotManager.getPlotTopLoc(Bukkit.getWorld(plot.world), plot.id);top.setY(PlotManager.getCoordinator().getHeightMaxLimit());
        Location bottom = PlotManager.getPlotBottomLoc(Bukkit.getWorld(plot.world), plot.id);bottom.setY(PlotManager.getCoordinator().getHeightMinLimit());
        
        PlotManager.getCoordinator().addPlayerRegion(getPlayerId(plot.owner), plot.id, new CrafterRegion(top, bottom));
    ...
    public static void addPlotAllowed(String player, UUID playerid, int idX, int idZ, String world) {
        ...
        Location top = PlotManager.getPlotTopLoc(Bukkit.getWorld(plot.world), plot.id);top.setY(PlotManager.getCoordinator().getHeightMaxLimit());
        Location bottom = PlotManager.getPlotBottomLoc(Bukkit.getWorld(plot.world), plot.id);bottom.setY(PlotManager.getCoordinator().getHeightMinLimit());
        PlotManager.getCoordinator().addPlayerRegion((playerid==null?getPlayerId(player):playerid), plot.id, new CrafterRegion(top, bottom));
    ...
    public static void deletePlot(int idX, int idZ, String world) {
    	for (CrafterPlayer player : PlotManager.getCoordinator().getPlayers().values()){
    		if (PlotManager.getCoordinator().getPlayerRegion(player.getId()).containsKey(idX+";"+idZ)){
    			PlotManager.getCoordinator().removePlayerRegion(player.getId(), idX+";"+idZ);}}
    ...
    public static void deletePlotAllowed(int idX, int idZ, String player, UUID playerid, String world) {
        ...
        if (playerid==null) offlinePlayer = Bukkit.getOfflinePlayer(playerid);
        CrafterPlayer crafterPlayer = PlotManager.getCoordinator().getPlayer(getPlayerId(player));
        if (PlotManager.getCoordinator().getPlayerRegion(crafterPlayer.getId()).containsKey(idX+";"+idZ)) PlotManager.getCoordinator().removePlayerRegion(crafterPlayer.getId(), idX+";"+idZ);
	...
	private static final long bufferTime = 60*1000;
    ...
    private static UUID getPlayerId(String playerName){
        ...
        if (player.getUniqueId() != null) {
            ...
            try {
                PlotMe.logger.info("Fetching " + playerName + " UUID from Mojang servers...");
                Map<String, UUID> response = fetcher.call();                    
                if (response.size() > 0) {
                	UUID playerId = response.values().toArray(new UUID[0])[0];
                	PlotMe.logger.info("Fetched " + playerId.toString() + " for " + playerName);
                    return playerId;}}
            catch (IOException e) {
            	...
            	PlotMe.logger.info("Exception while running UUIDFetcher");
                e.printStackTrace();}}
        return null;}
